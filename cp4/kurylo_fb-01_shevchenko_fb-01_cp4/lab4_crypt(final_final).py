...
def receive_key(k1, d1, n1, S1, e, k, S):
    print("\nОтримання повідомлення (k1, S1)")
    if k == pow(k1, d1, n1) and S == pow(S1, d1, n1):
        print("\nПеревірка конфеденційності успішна!")
        k == pow(k1, d1, n1)
        S == pow(S1, d1, n1)
    else:
        print("\nПеревірка конфеденційності хибна :(")
    if k == pow(S, e, n):
        print("\nПеревірка автентифікації успішна!")
        k == pow(S, e, n)
    else:
        print("\nПеревірка автентифікації хибна :(")
    return k, S
...
S = sign(M, d, n)
verify(S, e, n)
S1, k1, S = send_key(k, e1, n1, d, n)
k, S = receive_key(k1, d1, n1, S1, e, k, S)
